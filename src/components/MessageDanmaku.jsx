...
const MessageDanmaku = () => {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (e) => {
      ...
      setDisplayMessages(prev => [...prev, newDanmaku])
      
      // 7.125秒后移除弹幕
      setTimeout(() => {
        setDisplayMessages(prev => prev.filter(m => m.displayId !== newDanmaku.displayId))
      }, 7125)
    }
    
    window.addEventListener('showDanmaku', handleShow)
    window.addEventListener('newMessageSubmitted', handleNewMessage)
    
    return () => {
      window.removeEventListener('showDanmaku', handleShow)
      window.removeEventListener('newMessageSubmitted', handleNewMessage)
    }
  }, [])
